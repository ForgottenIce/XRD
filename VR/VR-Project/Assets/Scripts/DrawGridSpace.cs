...
public class DrawGridSpace : MonoBehaviour
{
    ...
    private void OnDrawGizmosSelected() {
        ...
        if (specificSpaceFloat != null && maze != null) {
        ...
        if (grid != null && maze != null) {
            ...
            for (int i = Math.Max(gridSpace.x - windowRadius + 1, 0); i < Math.Min(gridSpace.x + windowRadius, maze.Length); i++) {
                //for (int j = Math.Max(gridSpace.z - windowRadius + 1, 0); j < Math.Min(gridSpace.z + windowRadius, m_Spawner.maze[i].Length); j++) {
                for (int j = Math.Max(gridSpace.z - windowRadius + 1, 0); j < Math.Min(gridSpace.z + windowRadius, maze[i].Length); j++) {
                    if (i == Math.Max(gridSpace.x - windowRadius + 1, 0)
                        ...
                        || i == Math.Min(gridSpace.x + windowRadius, maze.Length) - 1
                        //|| j == Math.Min(gridSpace.z + windowRadius, m_Spawner.maze[i].Length) - 1) {
                        || j == Math.Min(gridSpace.z + windowRadius, maze[i].Length) - 1) {
                        Gizmos.color = Color.blue;
